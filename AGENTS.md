...
8. This fork is English only (see src/i18n/config.ts): `locales` holds just
   "en". The bilingual machinery still exists and still works exactly as
   documented in wiki/subsystems/i18n.md; adding "fr" back is a two line
   change, not a rewrite. Until then, do not add French content, and every
   page under [...locale]/ still needs `export const getStaticPaths =
   localePaths;` and every internal href still goes through
   `localizePath()`, both keep working correctly with one locale.
